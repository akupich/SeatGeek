...
struct EventsStorage: EventsStorageType {
    ...
    func delete(event: EventItem) -> Observable<Void> {
        let result = withRealm("deleting") { realm-> Observable<Void> in
            try realm.write {
                if let storedEvent = realm.object(ofType: EventItem.self, forPrimaryKey: event.eventId) {
    ...
    func find(event: EventItem) -> EventItem? {
        return withRealm("searching") { realm in
            return realm.object(ofType: EventItem.self, forPrimaryKey: event.eventId)
            } ?? nil
    }
